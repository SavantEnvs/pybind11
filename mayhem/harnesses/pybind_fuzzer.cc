...
// for the whole process and are intentionally never freed — the classic CPython
// LeakSanitizer noise. We deliberately do NOT bake a compiled-in sanitizer
// default-options override into the harness to silence it: per PE-7391 all ASan/
// LSan option passing belongs to Mayhem, which owns the runtime option set (and
// dispatched runs come back healthy with leak detection under Mayhem's control).
// The static scoped_interpreter below keeps that init to a single, process-lived
// allocation.
